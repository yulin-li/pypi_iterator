...
class TimeoutIterator:
    ...
    - max_buffered: maximum number of elements that can be buffered. Default=0 i.e. no limit
    ...
    Any exceptions raised within the wrapped iterator are propagate as it is.
    ...
    def __init__(self, iterator, timeout=0.0, sentinel=object(), reset_on_next=False, max_buffered=0):
    ...
    def __lookahead(self):
        try:
            while True:
                if self._max_buffered > 0 and self._buffer.qsize() > self._max_buffered:
                    time.sleep(0.01)
                else:
